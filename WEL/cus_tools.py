...
def cal_price_rate():
    print('Loading numerical dataset')
    #data = pd.read_csv('DJIA_table.csv')
    data = pd.read_csv('numerical_DJI.csv')
    
    close_price1=data['Adj Close']
    close_price2=close_price1
    first_day_change=11734.320313-11431.429688
    first_day_change_rate=first_day_change/11431.429688
    change_rate=[]
    change=[]
    change_rate.append(first_day_change_rate)
    change.append(first_day_change)
    for i in range(len(close_price1)-1):
        temp=close_price2[i+1]-close_price1[i]
        rate_temp=temp/close_price1[i]
        change_rate.append(rate_temp)
        change.append(temp)
        new_change_rate=[p*100 for p in change_rate]
    return change,new_change_rate

###for develop....
'''
price=cal_price_rate()[1]
